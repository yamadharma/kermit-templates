...
my %LAUNCH_COMMAND_LIST_ORIG = (
	'01_No prefered launcher, use first available' => 'none',
	'02_Gnome terminal' => 'gnome',
	'03_KDE console' => 'kde',
	'04_No graphical terminal' => 'stdout',
	);
...
sub initControls() : method {
	...
	# Build components
	my $filenameLabel = Gtk2::Label->new ($self->localeGet("Main TeX file:"));
	...
	my $preferedUILabel = Gtk2::Label->new ($self->localeGet("Prefered Launcher:"));
	...
	my $cleanButton = Gtk2::Button->new_with_label ($self->localeGet("Clean"));
	...
	my $cleanallButton = Gtk2::Button->new_with_label ($self->localeGet("Clean all"));
...
sub saveGUIConfiguration(\%) {
	my $self = shift;
	my $preferedUI = $self->attr('CONFIGURATION','gtk.preferred launcher');
	printDbgFor(4,"Preferred launcher: $preferedUI");
...
sub launchAutoLaTeX(@) : method {
	...
	$self->localeDbg("Launching AutoLaTeX with parameters: {}",join(' ',@_));
	printDbgIndent();
	...
	if (($preferedUI)&&(exists $LAUNCHERS{"$preferedUI"})) {
		$self->localeDbg("trying prefered launcher: {}", $preferedUI);
		my $command = $LAUNCHERS{"$preferedUI"};
		if (runSystemCommand($self,@{$command}, @_) == 0) {
			$self->localeDbg("launch succeeded");
			$succeed = 1;
		}
	...
	unless ($succeed) {
		foreach my $cmdlabel (@LAUNCH_COMMAND_ORDER) {
			if (exists $LAUNCHERS{"$cmdlabel"}) {
				$self->localeDbg("trying {}",$cmdlabel);
