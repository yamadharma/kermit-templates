...
sub getIcon($) {
	...
	printErr($self->localeGet("icon not found: {}", $iconName)) unless ($iconPath);
...
sub connectSignal($$$) : method {
	...
	my $proc = sub {
		...
		if ($refself->isAllowedSignal($refprocname,$refobj)) {
			$self->localeDbg("{}: on Gtk signal '{}', calling {}($)",scalar(localtime),$signame,$procname);
			...
			$self->localeDbg("Ignoring Gtk signal '{}'",$signame);
		}
	};
