...
sub new(\%\%\%\%) : method {
	...
	$self->set_title ("AutoLaTeX ".getAutoLaTeXVersion());
...
sub initializeDialogContent() : method {
	...
	$notebook->append_page($toolTab, $self->makeNotebookTab(
				$self->localeGet("Tools"), "tools.png"));
	...
	if ($self->hasProject ()) {
		...
		$notebook->append_page($projectConfTab, $self->makeNotebookTab(
				$self->localeGet("Project Configuration"), "projectLevel.png"));
	}
	...
	$notebook->append_page($userConfTab, $self->makeNotebookTab(
				$self->localeGet("User Configuration"), "userLevel.png"));
	...
	if ($self->isAdminUser ()) {
		...
		$notebook->append_page($systemConfTab, $self->makeNotebookTab(
				$self->localeGet("System Configuration"), "systemLevel.png"));
	}
...
sub onTranslatorPanelStateChanged($$) {
	...
	my $tabTitle;
	if ($conflict) {
		$tabTitle = $self->makeNotebookTab($self->localeGet("Translators"), "translators_err.png", "translators.png", "translators_err.png", "translators.png", "translators_err.png", "translators.png", "translators_err.png");
	}
	else {
		$tabTitle = $self->makeNotebookTab($self->localeGet("Translators"), "translators.png");
	}
...
sub onNotebookPageChanged($$) {
	...
	if ($oldpagenum>=0) {
		...
		if ($pageObj->can('savePanelContent')) {
			printDbgFor(3,"Saving hidden notebook panel");
