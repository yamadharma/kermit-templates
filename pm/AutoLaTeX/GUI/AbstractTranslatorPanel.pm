...
sub applyInclusionsInConfiguration(\%$) {
	...
	$self->localeDbg("No change detected") unless ($hasChanged);
	return $hasChanged;
}
...
sub savePanelContent() : method {
	...
	# System level
	if ($self->isAdminUser()) {
		$self->localeDbg("Saving system configuration about translators");
		printDbgIndent();
	...
	# User level
	$self->localeDbg("Saving user configuration about translators");
	printDbgIndent();
	...
	# Project level
	if ($self->hasProject()) {
		$self->localeDbg("Saving project configuration about translators");
		printDbgIndent();
